...
(function() {
    ...
    var MatchSummaryView = require('./matchSummaryView');
    ...
    var MatchListView = React.createClass({
        getInitialState: function() {
            return {
        ...
        handlePress: function(id) {
            ...
            var match =  _.find(this.props.matches, function(i) {
                return i.id === id;
            });
            ...
            console.log(matchSummaryView);

            console.log("IN HANDLE PRESS", id);


            if (match.matchOver) {
                this.props.renderScene(matchSummaryView);
                console.log(this.props.navigator);
                this.props.navigator.push(matchSummaryView);
            } else {
                this.props.renderScene(_matchView);
                this.props.navigator.push(_matchView);
            }
        },
                        ...
                        return ([
                            <View>
                                <View style={styles.outerRow} {...self._panGesture.panHandlers} onTouchStart={self.handleTouchStart.bind(null, data.id)} key={'key_' + data.id} ref={'parent_ref_' + data.id}>
                                    <View style={{flex: 1}} ref={'ref_' + data.id}>
                                        <TouchableHighlight style={{flex: 1}} onPress={self.handlePress.bind(null, data.id)}>
